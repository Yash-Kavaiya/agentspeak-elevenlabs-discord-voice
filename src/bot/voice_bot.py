"""Discord voice bot with ElevenLabs integration."""
...
from ..elevenlabs_agent.agent import ElevenLabsAgent, ConversationManager
from ..audio.discord_audio_source import ElevenLabsAudioSource
...
class VoiceBot(commands.Bot):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._active_conversations: Dict[int, ElevenLabsAgent] = {}
    ...
    async def on_ready(self) -> None:
        ...
        logger.info(f"ElevenLabs Agent ID: {self.elevenlabs_agent_id}")
    ...
    async def _join_voice_channel(self, channel: discord.VoiceChannel) -> None:
        ...
        try:
            ...
            # Set up callbacks
            agent.on_agent_speaking(
                lambda text: logger.info(f"[Agent]: {text}")
            )
            agent.on_user_speaking(
                lambda text: logger.info(f"[User]: {text}")
            )
            ...
            audio_source = ElevenLabsAudioSource(audio_interface.get_output_buffer())
            ...
            logger.info(f"ElevenLabs conversation started for {channel.name}")
...
def create_bot(
    ...
    **kwargs,
    ...
    Args:
        elevenlabs_agent_id: ElevenLabs agent ID (or from env)
        elevenlabs_api_key: ElevenLabs API key (or from env)
        **kwargs: Additional bot configuration

    Returns:
        Configured VoiceBot instance
    """
    agent_id = elevenlabs_agent_id or os.getenv("ELEVENLABS_AGENT_ID")
